...
Defines requirements for the backend `Settings` singleton entity and the `/api/v1/settings` endpoints that back the settings-app remote (workspace name, timezone, theme, notification preferences).
...
`SettingsController` SHALL KHÔNG áp dụng bất kỳ auth guard nào lên `/api/v1/settings` trong phase này.
