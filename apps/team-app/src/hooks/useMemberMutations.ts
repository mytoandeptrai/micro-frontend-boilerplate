...
async function createMember(payload: CreateMemberPayload): Promise<Member> {
  const result = await http.post<BaseResponseType<Member>>(
    "/api/v1/members",
    payload,
  )
  return result.data
...
async function updateMember(
  ...
  const result = await http.put<BaseResponseType<Member>>(
    `/api/v1/members/${id}`,
    payload,
  )
  return result.data
...
async function deleteMember(id: string): Promise<Member> {
  const result = await http.delete<BaseResponseType<Member>>(
    `/api/v1/members/${id}`,
  )
  return result.data
}
